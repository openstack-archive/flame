...
You need to implement the `bare_resources` property and the `get_resources()`
...
`post_process_resources(resources)`, `post_process_template(template)` and/or
`post_process_stack_data(stack_data)` methods to perform
